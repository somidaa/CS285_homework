...
class MLPPolicy(nn.Module):
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        obs = obs[None]             #在前面加一维，(1,ac_dim)
        obs = ptu.from_numpy(obs)   #转成tensor

        dist = self.forward(obs)    #取得动作分布

        action = dist.sample()      #从分布里采样一个动作

        action = ptu.to_numpy(action)[0]    #tensor转成numpy，去掉第0维
        return action
