...
def run_training(config: TrainConfig) -> None:
    ...
    for epoch in range(config.num_epochs):
        for obs_batch, expert_chunk_batch in loader:
            # 数据搬到 device（CPU/GPU）
            obs_batch = obs_batch.to(device)
            expert_chunk_batch = expert_chunk_batch.to(device)

            # 2) 前向 + 反向 + 更新
            model.train()
            loss = model.compute_loss(obs_batch, expert_chunk_batch)
    ...
    # 训练结束，做最后一次评估并导出 Gradescope 评分用日志
    evaluate_policy(
        model=model,
        normalizer=normalizer,
        device=device,
